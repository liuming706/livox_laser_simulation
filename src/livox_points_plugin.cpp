...
void LivoxPointsPlugin::Load(gazebo::sensors::SensorPtr _parent,
  ...
  //  rosPointPub = rosNode->advertise<sensor_msgs::PointCloud>(curr_scan_topic,
  //  5);
  customMsgPub =
...
void LivoxPointsPlugin::OnNewLaserScans() {
  if (rayShape) {
    ...
    livox_ros_driver2::CustomMsg
        pp_livox;  //声明要发布的 custom格式的livox 点云
    // 赋值点云帧头
    ...
    for (auto &pair : points_pair) {
      ...
      livox_ros_driver2::CustomPoint p;  //声明一个 Custom 点
      p.x = point.X();
      p.y = point.Y();
      p.z = point.Z();
      p.reflectivity = intensity;
      boost::chrono::high_resolution_clock::time_point end_time =
          boost::chrono::high_resolution_clock::now();  // 当前点时间
      boost::chrono::nanoseconds elapsed_time =
          boost::chrono::duration_cast<boost::chrono::nanoseconds>(end_time -
                                                                   start_time);
